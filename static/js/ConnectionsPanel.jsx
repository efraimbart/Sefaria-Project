...
class ConnectionsPanel extends Component {
  ...
  render() {
    ...
    if (!this.state.linksLoaded) {
      ...
      const chevrutaURL = `${window.location.host}/beit-midrash/${uuid}?ref=${window.location.pathname.replace(/\//, '')}`
...
const ResourcesList = ({ masterPanelMode, setConnectionsMode, counts }) => {
  ...
  return (
    <div className="toolButtonsList">
      ...
      <ToolsButton en="From the Rebbe" he="הרבי" image="webpages.svg" count={counts["webpages"]} urlConnectionsMode="WebPages" onClick={() => setConnectionsMode("WebPages")} />
